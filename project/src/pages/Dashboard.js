...
const Dashboard = () => {
    const { result } = (0, AssessmentContext_1.useAssessment)();
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="mb-8">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              <select value={selectedTimeframe} onChange={(e) => setSelectedTimeframe(e.target.value)} className="input text-sm">
        ...
        {/* Stats Grid */}
        <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {stats.map((stat, index) => (<div key={stat.title} className="card">
              <div className="flex items-center justify-between">
                <div>
        ...
        <framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="card mb-8">
          ...
          <recharts_1.ResponsiveContainer width="100%" height={300}>
            <recharts_1.LineChart data={mockProgressData}>
              <recharts_1.CartesianGrid strokeDasharray="3 3"/>
              <recharts_1.XAxis dataKey="date"/>
              <recharts_1.YAxis domain={[0, 100]}/>
              <recharts_1.Tooltip />
              <recharts_1.Line type="monotone" dataKey="anxiety" stroke="#ef4444" strokeWidth={2}/>
              <recharts_1.Line type="monotone" dataKey="depression" stroke="#f59e0b" strokeWidth={2}/>
              <recharts_1.Line type="monotone" dataKey="stress" stroke="#8b5cf6" strokeWidth={2}/>
              <recharts_1.Line type="monotone" dataKey="wellbeing" stroke="#22c55e" strokeWidth={2}/>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <framer_motion_1.motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="card">
            <h2 className="text-xl font-semibold text-secondary-900 mb-6 flex items-center">
              <lucide_react_1.Target className="h-6 w-6 mr-2 text-primary-600"/>
              Monthly Goals
            </h2>
            <div className="space-y-6">
              {mockGoals.map((goal) => (<div key={goal.id}>
                  ...
                  <div className="w-full bg-secondary-200 rounded-full h-2">
                    <framer_motion_1.motion.div className="bg-primary-600 h-2 rounded-full" initial={{ width: 0 }} animate={{ width: `${goal.progress}%` }} transition={{ duration: 0.8, delay: 0.2 }}/>
                  </div>
                </div>))}
            </div>
          </framer_motion_1.motion.div>
          ...
          {/* Recent Activities */}
          <framer_motion_1.motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.4 }} className="card">
            <h2 className="text-xl font-semibold text-secondary-900 mb-6 flex items-center">
              <lucide_react_1.Calendar className="h-6 w-6 mr-2 text-primary-600"/>
              Recent Activities
            </h2>
            <div className="space-y-4">
              {mockActivities.map((activity) => (<div key={activity.id} className="flex items-center space-x-4 p-3 bg-secondary-50 rounded-lg">
                  <div className={`w-3 h-3 rounded-full ${activity.completed ? 'bg-success-500' : 'bg-secondary-300'}`}/>
                  ...
                  <div className={`px-2 py-1 rounded text-xs font-medium ${activity.completed
        ...
        {result && (<framer_motion_1.motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.5 }} className="card">
            <h2 className="text-xl font-semibold text-secondary-900 mb-6 flex items-center">
              <lucide_react_1.Brain className="h-6 w-6 mr-2 text-primary-600"/>
              Latest Assessment Results
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {Object.entries(result.categoryScores).map(([category, score]) => (<div key={category} className="text-center">
